...
\noindent Cualquier capítulo puede tener múltiples apartados, como el @sec-lista-de-items o el @sec-enumeraciones de este mismo capítulo.

También está el @sec-primera-seccion del @sec-capitulo-dos que tiene la @fig-otra.

Es buena idea usar `\noindent` --que es un comando de \LaTeX{}-- al principio del primer párrafo, tras el encabezado de una sección o capítulo, para desactivar la sangría temporalmente.

## Listas de elementos {#sec-lista-de-items}

\noindent Esta la lista de elementos del @sec-lista-de-items:
...
\noindent Esto es una lista enumerada, que puede estar relacionada con la @fig-introduccion
...
\noindent En la @fig-introduccion se puede ver una figura de ejemplo.
...
\noindent En el @sec-apendice-uno se pueden observar varios ejemplos de entornos para describir algoritmos y código.

## Citas

\noindent Las referencias bibliográficas se deben indicar en el archivo `referencias.bib` y se citan en el texto.
